   ...
   struct termios * defaultAttribute = malloc (sizeof(struct termios));
    ...
    while(1){
            ...
            print_jobs(list_jobs);
                ...
                cmd_line* line;
                int number;
                line=parse_cmd_lines(input);
                number =atoi(line->arguments[1]);
                job * curr_job=find_job_by_index(job_list,number);
                if(curr_job!=NULL){
                    run_job_in_foreground(list_jobs, curr_job, 1, defaultAttribute,processId);
                }
            }
                ...
                line=parse_cmd_lines(input);
                number =atoi(line->arguments[1]);
                run_job_in_background(*list_jobs,number);
                }
            else{
